sdmTMB_map <- function(df, preds, title, lat_label){
  ...
  color_levels = 100
  max_absolute_value = max(abs(c(min(preds$preds_scaled_log, na.rm = T),
                                 max(preds$preds_scaled_log, na.rm = T))))
  color_sequence = seq(max(preds$preds_scaled_log, na.rm = T), 
                       min(preds$preds_scaled_log, na.rm = T),
                       length.out = color_levels + 1)
  n_in_class = hist(preds$preds_scaled_log, breaks = color_sequence, plot = F)$counts > 0
  ...
  image(lond,
        latd,
        t(matrix(preds$preds_scaled_log,
                 nrow = length(latd),
                 ncol = length(lond),
                 byrow = T)),
        xlim = c(-126, -116),
  ...
  image.plot(legend.only = T,
             ...
             legend.width = 0.8,
             legend.mar = 6,
             zlim = c(min(preds$preds_scaled_log, na.rm = T), 
                      max(preds$preds_scaled_log, na.rm = T)),
...
sdab_map <- function(df, preds, title, lat_label){
...
sdab_SVC <- function(df, preds, title, lat_label, var, variable){
