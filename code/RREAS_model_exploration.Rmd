...
catch_GAMs <- function(species_subset){
  ...
  oni_gam <- gam(lncatch ~ factor(year) + s(lon, lat) + s(bottom_depth, k = 4) + s(doy) + s(ONI, k = 4),
                  data = species_subset[species_subset$lncatch > 0,])
  pdo_flex <- gam(lncatch ~ factor(year) + s(lon, lat) + s(bottom_depth, k = 4) + s(doy) + s(lon, lat, by = PDO_pos),
                  data = species_subset[species_subset$lncatch > 0,])
  npgo_flex <- gam(lncatch ~ factor(year) + s(lon, lat) + s(bottom_depth, k = 4) + s(doy) + s(lon, lat, by = NPGO_pos),
                  data = species_subset[species_subset$lncatch > 0,])
  oni_flex <- gam(lncatch ~ factor(year) + s(lon, lat) + s(bottom_depth, k = 4) + s(doy) + s(lon, lat, by = ONI_pos),
                  data = species_subset[species_subset$lncatch > 0,])
...
tweedie_GAMs <- function(species_subset){
  ...
  oni_gam <- gam(catch + 1 ~ factor(year) + s(lon, lat) + s(bottom_depth, k = 4) + s(doy) + s(ONI, k = 4),
                  ...
                  method = 'REML',
                  data = species_subset)
  pdo_flex <- gam(catch + 1 ~ factor(year) + s(lon, lat) + s(bottom_depth, k = 4) + s(doy) + s(lon, lat, by = PDO_pos),
                  family = tw(link = "log"),
                  method = 'REML',
                  data = species_subset)
  npgo_flex <- gam(catch + 1 ~ factor(year) + s(lon, lat) + s(bottom_depth, k = 4) + s(doy) + s(lon, lat, by = NPGO_pos),
                  family = tw(link = "log"),
                  method = 'REML',
                  data = species_subset)
  oni_flex <- gam(catch + 1 ~ factor(year) + s(lon, lat) + s(bottom_depth, k = 4) + s(doy) + s(lon, lat, by = ONI_pos),
                  family = tw(link = "log"),
                  method = 'REML',
                  data = species_subset)
  gam_list <- list(year_gam, pdo_gam, npgo_gam, oni_gam, pdo_flex, npgo_flex, oni_flex)
...
```{r}
hake_lower <- gam(lower_cpue + 1 ~ factor(year) + s(lon, lat) + s(bottom_depth, k = 4) + s(doy) + s(lon, lat, by = NPGO_pos),
                  family = tw(link = "log"),
                  method = 'REML',
                  data = yoy_hake_filtered)
hake_upper <- gam(upper_cpue + 1 ~ factor(year) + s(lon, lat) + s(bottom_depth, k = 4) + s(doy) + s(lon, lat, by = NPGO_pos),
                  family = tw(link = "log"),
                  method = 'REML',
                  data = yoy_hake_filtered)
summary(hake_lower)
