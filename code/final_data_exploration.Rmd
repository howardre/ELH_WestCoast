...
clean_catch <- function(data_list, i){
  ...
  data_frame <- data_frame %>% filter(active == 'Y') %>%
    mutate(numeric_date = as.numeric(format(as.POSIXct(haul_date, 
                                                      format = "%Y-%m-%d"), 
                                           "%m%d%Y")))
  return(data_frame)
}
...
yoy_anch <- merge_lengths(anch_catch, haul_data, anch_subset, RREAS_ctd_means, PRS_avg_ctds)
yoy_widw <- merge_lengths(widow_catch, haul_data, widow_subset, RREAS_ctd_means, PRS_avg_ctds)
yoy_sbly <- merge_lengths(sbly_catch, haul_data, sbly_subset, RREAS_ctd_means, PRS_avg_ctds)
yoy_dab <- merge_lengths(dab_catch, haul_data, dab_subset, RREAS_ctd_means, PRS_avg_ctds)
yoy_hake <- merge_lengths(hake_catch, haul_data, hake_subset, RREAS_ctd_means, PRS_avg_ctds)
```
