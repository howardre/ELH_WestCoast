...
hindcast_match <- function(data, roms){
  data[, c(36, 37)] <- as.data.frame(RANN::nn2(roms[, c('lat', 'lon', 'numeric_date')],
  ...
  data <- data[-c(36, 37)] # remove extra columns before predicting
}
```
...
yoy_anch <- hindcast_match(merge_anch, hindcast_trim)
yoy_dab <- hindcast_match(merge_dab, hindcast_trim)
yoy_hake <- hindcast_match(merge_hake, hindcast_trim)
yoy_sbly <- hindcast_match(merge_sbly, hindcast_trim)
yoy_widw <- hindcast_match(merge_widw, hindcast_trim)
```
