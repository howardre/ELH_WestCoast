...
hindcast_month <- aggregate(list(month_zeta = hindcast_depths$zeta, 
                                 month_temp = hindcast_depths$temp, 
                                 month_salt = hindcast_depths$salt), 
                            by = list(month = hindcast_depths$month, 
                                      year = hindcast_depths$year), 
                            mean)
...
hindcast_year <- hindcast_month %>%
  ...
  group_by(year) %>%
  mutate(mean_ssh = mean(month_zeta, na.rm = TRUE),
         mean_temp = mean(month_temp, na.rm = TRUE),
         mean_salt = mean(month_salt, na.rm = TRUE))

# Match back to the main data frame
hindcast_trim$mean_ssh <- hindcast_year$mean_ssh[match(hindcast_trim$year, 
                                                       hindcast_year$year)]
hindcast_trim$mean_temp <- hindcast_year$mean_temp[match(hindcast_trim$year, 
                                                       hindcast_year$year)]
hindcast_trim$mean_salt <- hindcast_year$mean_salt[match(hindcast_trim$year, 
                                                       hindcast_year$year)]

hindcast_means <- merge(hindcast_trim,
                        hindcast_month,
                        by = c('month',
                               'year'),
                        all.x = T)
...
hindcast_anom <- hindcast_means %>%
  mutate(ssh_anom_m = zeta - month_zeta,
         ssh_anom_y = zeta - mean_ssh,
         sst_anom_m = temp - month_temp,
         sst_anom_y = temp - mean_temp,
         sss_anom_m = salt - month_salt,
         sss_anom_y = salt - mean_salt)
```
...
hindcast_means <- hindcast_trim %>%
  group_by(year) %>%
  mutate(mean_ssh = mean(zeta, na.rm = TRUE),
         mean_temp = mean(temp, na.rm = TRUE))

saveRDS(hindcast_means, file = "../data/hindcast_means.Rdata")
```
