...
merge_lengths <- function(catch, hauls, subset, RREAS_means, PRS_means){
  ...
  data_final <- data_comb[-c(36, 37, 38)]
  return(data_final)
}
...
size_calc <- function(sp, size){
  ...
  subset <- sizes %>% 
    dplyr::group_by(numeric_date, station, haul_no, cruise, haul_exp, size_class) %>%
...
hindcast_match <- function(data, roms, a, b){
  data[, c(a, b)] <- as.data.frame(RANN::nn2(roms[, c('lat', 'lon', 'numeric_date')],
                                                   data[, c('lat', 'lon', 'numeric_date')],
                                                   k = 1))
  data$roms_temperature <- as.data.frame(roms)[c(data$nn.idx), 5] # Match nearest temp
  data$roms_salinity <- as.data.frame(roms)[c(data$nn.idx), 4] # Match nearest temp
  data$roms_ssh <- as.data.frame(roms)[c(data$nn.idx), 6]
  data$mean_temp <- as.data.frame(roms)[c(data$nn.idx), 16]
  data$mean_ssh <- as.data.frame(roms)[c(data$nn.idx), 15]
...
widow_subset <- size_calc(widow_sp, 45) # reevaluate this one
sbly_subset <- size_calc(sbly_sp, 35)
dab_subset <- size_calc(dab_sp, 30)
...
fillvalue_s <- ncatt_get(nc, 'salt', "_FillValue")
fillvalue_h <- ncatt_get(nc, 'zeta', "_FillValue")
temp <- ncvar_get(nc, varid = 'temp')
temp[temp == fillvalue_t$value] <- NA
salt <- ncvar_get(nc, varid = 'salt')
salt[salt == fillvalue_s$value] <- NA
zeta <- ncvar_get(nc, varid = 'zeta')
zeta[zeta == fillvalue_h$value] <- NA

nc_close(nc)
```

```{r}
temp_avg <- apply(temp[, 1, 1:13], c(1, 2), mean, na.rm = TRUE)
temp_sd <- apply(temp[, 1, 1:13], c(1, 2), sd, na.rm = TRUE)
```


```{r}
library(raster)
nc_salt <- brick(here('data','nep_srf_1995-2020.nc'),
                 varname = "salt")
nc_temp <- brick(here('data','nep_srf_1995-2020.nc'),
                 varname = "temp")
nc_zeta <- brick(here('data','nep_srf_1995-2020.nc'),
                 varname = "zeta")

months <- as.integer(format(time1, "%m"))

# Calculate monthly mean for each variable at each location
salt_means <- stackApply(nc_salt, months, fun = mean) # removing NA's changes size of layers
temp_means <- stackApply(nc_temp, months, fun = mean)
zeta_means <- stackApply(nc_zeta, months, fun = mean)

# Calculate monthly standard deviation for each variable at each location
salt_sd <- stackApply(nc_salt, months, fun = sd)
temp_sd <- stackApply(nc_temp, months, fun = sd)
zeta_sd <- stackApply(nc_zeta, months, fun = sd)

# Calculate anomalies
salt_brick <- (nc_salt - salt_means) / salt_sd
temp_brick <- (nc_temp - temp_means) / temp_sd
zeta_brick <- (nc_zeta - zeta_means) / zeta_sd
```

```{r}
hindcast_sp <- nep_hindcast
coordinates(hindcast_sp) <- ~ lon + lat
gridded(hindcast_sp) <- TRUE

extent <- terra::ext(apply(hindcast_trim[, 7:8], 2, range))
raster <- terra::rast(extent, ncol = 10, nrow = 2)
test <- terra::rasterize(hindcast_trim[, 7:8],
                         raster,
                         hindcast_trim[, 4],
                         fun = mean)
```
...
```{r}
hindcast_rast <- rasterFromXYZ(hindcast_trim)

# Calculate monthly mean for each variable at each location
salt_means <- stackApply(nc_salt, months, fun = mean, na.rm = TRUE)
temp_means <- stackApply(nc_temp, months, fun = mean, na.rm = TRUE)
zeta_means <- stackApply(nc_zeta, months, fun = mean, na.rm = TRUE)

# Calculate monthly standard deviation for each variable at each location
salt_sd <- stackApply(nc_salt, months, fun = sd, na.rm = TRUE)
temp_sd <- stackApply(nc_temp, months, fun = sd, na.rm = TRUE)
zeta_sd <- stackApply(nc_zeta, months, fun = sd, na.rm = TRUE)
```
...
hindcast_trim[, c(15, 16)] <- as.data.frame(RANN::nn2(depth_df[, c('y', 'x')],
                                                      hindcast_trim[, c('lat', 'lon')],
                                                      k = 1))
hindcast_trim$depth <- as.data.frame(depth_df)[c(hindcast_trim$nn.idx), 3] 
hindcast_trim <- hindcast_trim[-c(15, 16)]

# Remove the values not on the shelf
hindcast_depths <- filter(hindcast_trim,
                          between(depth, -100, 0)) 

# Calculate average monthly values per year
hindcast_month <- hindcast_depths %>%
  group_by(month) %>%
  mutate(month_ssh = mean(zeta, na.rm = TRUE),
         month_temp = mean(temp, na.rm = TRUE),
         month_salt = mean(salt, na.rm = TRUE))

# Calculate average values for each year between June and August
ssh_avg <- hindcast_month %>%
  filter(between(month, 6, 8)) %>%
  summarise(mean_ssh = mean(zeta, na.rm = TRUE),
            mean_temp = mean(temp, na.rm = TRUE),
            mean_salt = mean(salt, na.rm = TRUE)) %>%
  summarise(mean_ssh = mean(mean_ssh, na.rm = TRUE),
            mean_temp = mean(mean_temp, na.rm = TRUE),
            mean_salt = mean(mean_salt, na.rm = TRUE))

hindcast_year <- hindcast_month %>%
  filter(between(month, 6, 8)) %>%
  group_by(year) %>%
  summarise(year_ssh = mean(zeta, na.rm = TRUE),
            year_temp = mean(temp, na.rm = TRUE),
            year_salt = mean(salt, na.rm = TRUE)) %>%
  mutate(ssh_anom = year_ssh - ssh_avg$mean_ssh,
         sst_anom = year_temp - ssh_avg$mean_temp,
         sss_anom = year_salt - ssh_avg$mean_salt)


# Match back to the main data frame
hindcast_trim$month_ssh <- hindcast_month$month_ssh[match(hindcast_trim$month, 
                                                         hindcast_month$month)]
hindcast_trim$month_temp <- hindcast_month$month_temp[match(hindcast_trim$month, 
                                                           hindcast_month$month)]
hindcast_trim$month_salt <- hindcast_month$month_salt[match(hindcast_trim$month, 
                                                           hindcast_month$month)]
hindcast_trim$ssh_anom <- hindcast_year$ssh_anom[match(hindcast_trim$year, 
                                                         hindcast_year$year)]
hindcast_trim$sst_anom <- hindcast_year$sst_anom[match(hindcast_trim$year, 
                                                           hindcast_year$year)]
hindcast_trim$sss_anom <- hindcast_year$sss_anom[match(hindcast_trim$year, 
                                                           hindcast_year$year)]

# Likely going to remove the 
hindcast_anom <- hindcast_trim %>%
  mutate(ssh_anom_m = zeta - month_ssh,
         sst_anom_m = temp - month_temp,
         sss_anom_m = salt - month_salt)
...
yoy_anch <- hindcast_match(merge_anch, hindcast_means, 36, 37)
yoy_dab <- hindcast_match(merge_dab, hindcast_means, 37, 38)
yoy_hake <- hindcast_match(merge_hake, hindcast_means, 37, 38)
yoy_sbly <- hindcast_match(merge_sbly, hindcast_means, 37, 38)
yoy_widw <- hindcast_match(merge_widw, hindcast_means, 37, 38)
```
