...
merge_lengths <- function(catch, hauls, subset){
  ...
  data_id <- merge(hauls[, c("haul_no", "cruise")],
                   data_zero,
                   by = c("haul_no", "cruise"),
                   all.y = TRUE)
  return(data_id)
...
yoy_anch <- hindcast_match(merge_anch, hindcast_anom, 23, 24)
yoy_dab <- hindcast_match(merge_dab, hindcast_anom, 24, 25)
yoy_hake <- hindcast_match(merge_hake, hindcast_anom, 24, 25)
yoy_sbly <- hindcast_match(merge_sbly, hindcast_anom, 24, 25)
yoy_widw <- hindcast_match(merge_widw, hindcast_anom, 24, 25)
```
