...
#   # Add ROMS hindcast
#   nc <- nc_open(here('data', 'NEP_variables_AvgMarchMay_1995-2020.nc'))
#   lat <- ncvar_get(nc, "latitude")
#   vgeo <- ncvar_get(nc, "vgeo") # surface geostrophic v-component avg from coast to 50 km offshore
#   uv_50 <- ncvar_get(nc, "u_vint_50m")
#   uv_100 <- ncvar_get(nc, "u_vint_100m")
#   iso26 <- ncvar_get(nc, "depth_iso26")
#   spice <- ncvar_get(nc, "spice_iso26")
#   v_undercurrent <- ncvar_get(nc, "v_cu")
#   vmax_undercurrent <- ncvar_get(nc, "vmax_cu")
...
#       match_indices <- which(nearby_lat_indices == TRUE)
#       
...
#       tows$vgeo = vgeo[row, column, date_index]
#       tows$uv50 = uv50[row, column, date_index]
#       tows$uv_100 = uv_100[row, column, date_index]
#       tows$iso26 = iso26[row, column, date_index]
#       tows$spice = spice[row, column, date_index]
#       tows$v_undercurrent = v_undercurrent[row, column, date_index]
#       tows$vmax_undercurrent = vmax_undercurrent[row, column, date_index]
# 
...
test <- merge_anch
years <- unique(merge_anch$year)
for(i in years){
  test[, c(25, 26)] <- as.data.frame(RANN::nn2(hindcast_anom[, c('lat', 'lon')][i],
                                           test[, c('lat', 'lon')][i],
                                           k = 1))
}
  
  
test[, c(25, 26)] <- as.data.frame(RANN::nn2(hindcast_anom[, c('lat', 'lon')],
                                           test[, c('lat', 'lon')],
                                           k = 1))
data$roms_temperature <- as.data.frame(roms)[c(data$nn.idx), 5] # Match nearest temp
data$roms_salinity <- as.data.frame(roms)[c(data$nn.idx), 4]
