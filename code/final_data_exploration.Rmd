...
merge_lengths <- function(catch, hauls, subset, RREAS_means, PRS_means){
  ...
  data_ctd <- merge(data_id,
                    RREAS_means,
                    by = c('cruise', 'ctd_index', 'year')) # match RREAS ctds
...
other_catch[col_names_catch] <- sapply(other_catch[col_names_catch], as.numeric) # changes NA's to actual NA
...
other_sp[col_names_sp] <- sapply(other_sp[col_names_sp], as.numeric) # Creates actual NA's
...
years_list <- as.character(c(1993:2014))
get_filepath <- function(years){
  filepath = paste('C:/Users/howar/Documents/Oregon State/ELH_WestCoast/data/aviso_tg_ssh/osu_daily_sla_geovel_',
                   years, '_v2.nc.gz', sep = '')
  return(filepath)
}
...
ssh_var <- list()
for(j in 1:22){
  model_ssh <- nc_open(get_filepath(years_list[j]))
  ssh_output <- nc_extract(model_ssh, 
                           sea_surface_height_above_sea_level, 
                           'sea_surface_height_above_sea_level')
  nc_close(model_ssh)
  gc()
  ssh_var[[paste("ssh_output", j, sep = "")]] <- ssh_output
}

ssh_combined <- list()
for(j in 1:22){
  model_ssh <- nc_open(get_filepath(years_list[j]))
  ssh_output <- cbind(ncvar_get(model_ssh, 'sea_surface_height_above_sea_level'))
  var <- model_ssh$var[]
  nc_close(model_ssh)
  gc()
  ssh_var[[paste("ssh_output", j, sep = "")]] <- ssh_output
...
yoy_anch <- hindcast_match(merge_anch, hindcast_anom, 28, 29)
yoy_dab <- hindcast_match(merge_dab, hindcast_anom, 29, 30)
yoy_hake <- hindcast_match(merge_hake, hindcast_anom, 29, 30)
yoy_sbly <- hindcast_match(merge_sbly, hindcast_anom, 29, 30)
yoy_widw <- hindcast_match(merge_widw, hindcast_anom, 29, 30)
```
