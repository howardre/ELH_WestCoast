...
catch_GAMs <- function(species_subset){
  ...
  ts_gam <- gam(lncpue ~ factor(year) + s(longitude, latitude) + s(bottom_depth, k = 4) + s(julian) +
                ...
                data = species_subset[species_subset$lncpue > 0,])  
  ts_NPGO_gam <- gam(lncpue ~ factor(year) + s(longitude, latitude) + s(bottom_depth, k = 4) + s(julian) +
                       s(temperature, k = 4) + s(salinity, k = 4) + s(longitude, latitude, by = NPGO_pos),
                     data = species_subset[species_subset$lncpue > 0,])    
  ts_PDO_gam <- gam(lncpue ~ factor(year) + s(longitude, latitude) + s(bottom_depth, k = 4) + s(julian) +
                       s(temperature, k = 4) + s(salinity, k = 4) + s(longitude, latitude, by = PDO_pos),
                     data = species_subset[species_subset$lncpue > 0,])   
  ts_ONI_gam <- gam(lncpue ~ factor(year) + s(longitude, latitude) + s(bottom_depth, k = 4) + s(julian) +
                       s(temperature, k = 4) + s(salinity, k = 4) + s(longitude, latitude, by = ONI_pos),
                     data = species_subset[species_subset$lncpue > 0,])   
  gam_list <- list(base_gam, ts_gam, ts_NPGO_gam, ts_PDO_gam, ts_ONI_gam)
...
tweedie_GAMs <- function(species_subset){
  ...
  ts_tw <- gam(catch + 1 ~ factor(year) + s(longitude, latitude) + s(bottom_depth, k = 4) + s(julian) +
               ...
               method = 'REML',
               data = species_subset)   
  ts_NPGO_tw <- gam(catch + 1 ~ factor(year) + s(longitude, latitude) + s(bottom_depth, k = 4) + s(julian) +
                      s(temperature, k = 4) + s(salinity, k = 4) + s(longitude, latitude, by = NPGO_pos),
                    family = tw(link = "log"),
                    method = 'REML',
                    data = species_subset)    
  ts_PDO_tw <- gam(catch + 1 ~ factor(year) + s(longitude, latitude) + s(bottom_depth, k = 4) + s(julian) +
                      s(temperature, k = 4) + s(salinity, k = 4) + s(longitude, latitude, by = PDO_pos),
                    family = tw(link = "log"),
                    method = 'REML',
                    data = species_subset) 
  ts_ONI_tw <- gam(catch + 1 ~ factor(year) + s(longitude, latitude) + s(bottom_depth, k = 4) + s(julian) +
                      s(temperature, k = 4) + s(salinity, k = 4) + s(longitude, latitude, by = ONI_pos),
                    family = tw(link = "log"),
                    method = 'REML',
                    data = species_subset) 
  gam_list <- list(base_tw, ts_tw, ts_NPGO_tw, ts_PDO_tw, ts_ONI_tw)
  best_gam <- gam_list[[which.min(sapply(1:length(gam_list),
                                         function(x) min(gam_list[[x]]$aic)))]] 
  return_list <- list(gam_list, best_gam)
} 
...
plot_var_coef <- function(my_gam, data, predictions){
  par(mar = c(6.4, 7.2, .5, 0.6) + 0.1,
      oma = c(1, 1, 1, 1),
      mgp = c(5, 2, 0))
  myvis_gam(my_gam,
...
```{r}
# Load data
yoy_hake <- readRDS(file = "../data/yoy_hake.Rdata") %>% drop_na(temperature, salinity, bottom_depth)
...
Match with CTD means (uncomment if change to mean temperature for VC term)
```{r}
...
hake_base <- gam(lncpue ~ factor(year) +
                   ...
                   s(bottom_depth, k = 4),
                 data = yoy_hake[yoy_hake$lncpue > 0,])
summary(hake_base)
...
anch_base <- gam(lncpue ~ factor(year) +
                  ...
                  s(longitude, latitude) +
                  s(bottom_depth, k = 4),
                data = yoy_anch[yoy_anch$lncpue > 0,])
summary(anch_base)
...
widow_base <- gam(lncpue ~ factor(year) +
                  ...
                  s(longitude, latitude) +
                  s(bottom_depth, k = 4),
                data = yoy_widow[yoy_widow$lncpue > 0,])
summary(widow_base)
...
# Select best GAM
widow_gam <- catch_GAMs(yoy_widow)
summary(widow_gam[[2]])
...
sbelly_base <- gam(lncpue ~ factor(year) +
                  ...
                  s(longitude, latitude) +
                  s(bottom_depth, k = 4),
                data = yoy_sbelly[yoy_sbelly$lncpue > 0,])
summary(sbelly_base)
...
# Select best GAM
sbelly_gam <- catch_GAMs(yoy_sbelly)
...
# Select best GAM
hake_gam_t <- tweedie_GAMs(yoy_hake)
...
```{r}
pred_hake_t <- variable_coefficient(hake_gam_t, yoy_hake, yoy_hake$NPGO_pos)

plot_var_coef(hake_gam_t[[2]], yoy_hake, pred_hake_t)
dev.copy(jpeg, here::here('results/yoy_hake_preliminary', 'yoy_hake_var_coef.jpg'), 
...
plot_variable(hake_gam_t, 
              ...
              "", 
              "n")
dev.copy(jpeg, here('results/yoy_hake_preliminary', 'yoy_hake_temp.jpg'), 
         height = 10, width = 10, units = 'in', res = 200)
dev.off()
```
...
plot_variable(hake_gam_t, 
              ...
              "", 
              "n")
dev.copy(jpeg, here('results/yoy_hake_preliminary', 'yoy_hake_salinity.jpg'), 
         height = 10, width = 10, units = 'in', res = 200)
dev.off()
```
...
Split into two models (use model selected for all data)
```{r}
hake_lower <- gam(lower_cpue + 1 ~ factor(year) + s(longitude, latitude) + s(bottom_depth, k = 4) + 
                    s(julian) + s(temperature) + s(salinity) + s(longitude, latitude, by = NPGO_pos),
                  family = tw(link = "log"),
                  method = 'REML',
                  data = yoy_hake)
hake_upper <- gam(upper_cpue + 1 ~ factor(year) + s(longitude, latitude) + s(bottom_depth, k = 4) + s(julian) + s(longitude, latitude, by = NPGO_pos),
                  family = tw(link = "log"),
                  method = 'REML',
                  data = yoy_hake)
summary(hake_lower)
...
```{r}
pred_yoy_hake_lower <- variable_coefficient_lengths(hake_lower, yoy_hake, yoy_hake_filtered$NPGO_pos)

plot_var_coef_lengths(hake_lower, yoy_hake, pred_yoy_hake_lower)
...
```{r}
pred_yoy_hake_upper <- variable_coefficient_lengths(hake_upper, yoy_hake, yoy_hake_filtered$NPGO_pos)

plot_var_coef_lengths(hake_upper, yoy_hake, pred_yoy_hake_upper)
