...
catch_GAMs <- function(species_subset){
  ...
  tso_phen_gam <- gam(lncpue ~ factor(year) + s(longitude, latitude) + s(bottom_depth, k = 4) + s(julian) +
                      ...
                      data = species_subset[species_subset$lncpue > 0,])    
  gam_list <- list(base_gam, ts_gam, tso_gam, ts_geog_gam, tso_geog_gam, ts_phen_gam, tso_phen_gam)
...
tweedie_GAMs <- function(species_subset){
  year_gam <- gam(catch + 1 ~ factor(year) + s(lon, lat) + s(bottom_depth, k = 4) + s(julian),
                  family = tw(link = "log"),
                  method = 'REML',
                  data = species_subset)
  pdo_gam <- gam(catch + 1 ~ factor(year) + s(lon, lat) + s(bottom_depth, k = 4) + s(julian) + s(PDO, k = 4),
                  family = tw(link = "log"),
                  method = 'REML',
                  data = species_subset)
  npgo_gam <- gam(catch + 1 ~ factor(year) + s(lon, lat) + s(bottom_depth, k = 4) + s(julian) + s(NPGO, k = 4),
                  family = tw(link = "log"),
                  method = 'REML',
                  data = species_subset)
  oni_gam <- gam(catch + 1 ~ factor(year) + s(lon, lat) + s(bottom_depth, k = 4) + s(julian) + s(ONI, k = 4),
                  family = tw(link = "log"),
                  method = 'REML',
                  data = species_subset)
  pdo_flex <- gam(catch + 1 ~ factor(year) + s(lon, lat) + s(bottom_depth, k = 4) + s(julian) + s(longitude, latitude, by = PDO_pos),
                  family = tw(link = "log"),
                  method = 'REML',
                  data = species_subset)
  npgo_flex <- gam(catch + 1 ~ factor(year) + s(lon, lat) + s(bottom_depth, k = 4) + s(julian) + s(longitude, latitude, by = NPGO_pos),
                  family = tw(link = "log"),
                  method = 'REML',
                  data = species_subset)
  oni_flex <- gam(catch + 1 ~ factor(year) + s(lon, lat) + s(bottom_depth, k = 4) + s(julian) + s(longitude, latitude, by = ONI_pos),
                  family = tw(link = "log"),
                  method = 'REML',
                  data = species_subset)
...
```{r}
# Base GAM
adult_dab_base <- gam(lncpue ~ factor(year) +
                        s(julian) +
                        s(longitude, latitude) +
                        s(bottom_depth, k = 4),
                      data = adult_dab_catch[adult_dab_catch$lncpue > 0,])
...
plot_var_coef(hake_gam_t, yoy_hake, pred_hake_t)
