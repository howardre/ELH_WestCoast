...
add_lengths <- function(species, split, lengths){
  lower <- aggregate(std_length ~ haul_no + cruise,
                         data = lengths,
                         FUN = length,
                         subset = std_length <= split)
...
climate_match <- function(data){
  ...
  data$NPGO_mean <- npgo_means$NPGO[match_npgo]
  match_pdo <- match(data$year, pdo_means$year)
  data$PDO_mean <- pdo_means$PDO[match_pdo]
  match_oni <- match(data$year, oni_means$year)
  data$ONI_mean <- oni_means$ONI[match_oni]
...
```{r}
ggplot(RREAS_hake) +
  geom_point(aes(catch, as.numeric(bottom_depth)),
             alpha = 0.1,
             size = 2)
RREAS_hake['catch'][RREAS_hake['catch'] > 4000] <- 4000
```

```{r}
ggplot(RREAS_sanddab) +
    geom_point(aes(catch, as.numeric(bottom_depth)),
             alpha = 0.1,
             size = 2)
RREAS_sanddab['catch'][RREAS_sanddab['catch'] > 1250] <- 1250
```

```{r}
ggplot(RREAS_anchovy) +
    geom_point(aes(catch, as.numeric(bottom_depth)),
             alpha = 0.1,
             size = 2)
RREAS_anchovy['catch'][RREAS_anchovy['catch'] > 7000] <- 7000
```
...
ggplot(RREAS_widow) +
    geom_point(aes(catch, as.numeric(bottom_depth)),
             alpha = 0.1,
             size = 2)
RREAS_widow['catch'][RREAS_widow['catch'] > 1000] <- 1000
```

```{r}
ggplot(RREAS_shortbelly) +
    geom_point(aes(catch, as.numeric(bottom_depth)),
             alpha = 0.1,
             size = 2)
RREAS_shortbelly['catch'][RREAS_shortbelly['catch'] > 5000] <- 5000
```
...
npgo_filtered <- npgo_index %>%
  filter(MONTH >= 1 & MONTH <= 3) %>%
  select(YEAR, NPGO) 
npgo_means <- aggregate(NPGO ~ YEAR, npgo_filtered, mean)

pdo_filtered <- pdo_index %>%
  filter(month >= 1 & month <= 3) %>%
  select(year, PDO) 
pdo_means <- aggregate(PDO ~ year, pdo_filtered, mean)

oni_filtered <- oni_matrix %>%
  filter(month >= 1 & month <= 3) %>%
  select(year, ONI) 
oni_means <- aggregate(ONI ~ year, oni_filtered, mean)
```

Add climate indices
```{r}
RREAS_list_clim <- lapply(RREAS_list, function(x) climate_match(x))
```

Add CTD to abundance data
```{r}
RREAS_list_env <- lapply(RREAS_list_clim, function(x) merge(x, ctd_means,
                                                             by = c('cruise',
                                                                    'ctd_index',
                                                                    'year'),
                                                             all.x = T))
...
yoy_hake <- add_lengths(RREAS_list_env[[1]], 31, length_list[[1]]) # Phillips et al. (2007)
yoy_sanddab <- add_lengths(RREAS_list_env[[2]], 30, length_list[[2]]) # Sakuma (1995)
yoy_anchovy <- add_lengths(RREAS_list_env[[3]], 35, length_list[[3]]) # Butler et al. (1993)
yoy_widow <- add_lengths(RREAS_list_env[[4]], 31, length_list[[4]]) # Laroche & Richardson (1981)
yoy_shortbelly <- add_lengths(RREAS_list_env[[5]], 25, length_list[[5]]) # Norton et al. (2001)
```
